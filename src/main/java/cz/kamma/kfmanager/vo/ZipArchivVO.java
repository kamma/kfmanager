...
public class ZipArchivVO {
	...
	public FileItemVO[] getFilesByPath(String path, boolean withParent) {
		...
		Vector<String> tmpDirNames = new Vector<>();
		if (withParent)
		...
		for (Enumeration en = zipFile.entries(); en.hasMoreElements();) {
			...
			if (ze.getName().startsWith(path) && !ze.getName().equals(path)) {
				...
				if (nameParts.length < 2 && !ze.isDirectory())
					res.add(new FileItemVO(FileHelper.getFileNameFromZip(ze.getName()), ze.getName(),
							ze.isDirectory() ? Constants.FILE_TYPE_DIR : Constants.FILE_TYPE_FILE, ze.getTime(),
							ze.getSize(), true, zipFilePath));
				if (ze.isDirectory()) {
					String absPath = path + nameParts[0] + Constants.ZIP_PATH_SEPARATOR;
					if (!tmpDirNames.contains(absPath)) {
						tmpDirNames.add(absPath);
						res.add(new FileItemVO(FileHelper.getFileNameFromZip(absPath), absPath,
								ze.isDirectory() ? Constants.FILE_TYPE_DIR : Constants.FILE_TYPE_FILE, ze.getTime(),
								ze.getSize(), true, zipFilePath));
					}
				}
	...
	private FileItemVO[] getRootFiles() {
		...
		for (Enumeration en = zipFile.entries(); en.hasMoreElements();) {
			ZipEntry ze = (ZipEntry) en.nextElement();
			if (!ze.getName().contains(Constants.ZIP_PATH_SEPARATOR)
					|| (getCharCount(ze.getName(), Constants.ZIP_PATH_SEPARATOR_CHAR) == 1
							&& ze.getName().endsWith(Constants.ZIP_PATH_SEPARATOR)))
				res.add(new FileItemVO(FileHelper.getFileNameFromZip(ze.getName()), ze.getName(),
						ze.isDirectory() ? Constants.FILE_TYPE_DIR : Constants.FILE_TYPE_FILE, ze.getTime(),
						ze.getSize(), true, zipFilePath));
		}
	...
	public InputStream getInputStream(String fileName) {
		...
		try {
			...
			System.out.println("ZipArchivVO[getInputStream]: Cannot get InputStream for file: " + fileName + " (ex: "
					+ e.getMessage() + ")");
		}
		return null;
	}
