...
public class MainWindow extends Constants {
	...
	public Shell open(Display display) {
		...
		shell.addListener(SWT.Resize, new Listener() {
			...
			public void handleEvent(Event e) {
				if (!ConfigHelper.getInstance()
						.getProperty(
								PreferenceManager.SETTINGS_PREFIX_LEFT + PreferenceManager.SETTINGS_PANELS_SHOW_TYPE,
								"Full")
						.equalsIgnoreCase("full"))
					leftPanel.getColumn(0).setWidth(leftLabel.getBounds().width - 4);
				if (!ConfigHelper.getInstance()
						.getProperty(
								PreferenceManager.SETTINGS_PREFIX_RIGHT + PreferenceManager.SETTINGS_PANELS_SHOW_TYPE,
								"Full")
						.equalsIgnoreCase("full"))
		...
		if (!ConfigHelper.getInstance()
				.getProperty(PreferenceManager.SETTINGS_PREFIX_LEFT + PreferenceManager.SETTINGS_PANELS_SHOW_TYPE,
						"Full")
				.equalsIgnoreCase("full"))
			leftPanel.getColumn(0).setWidth(leftLabel.getBounds().width - 4);
		if (!ConfigHelper.getInstance()
				.getProperty(PreferenceManager.SETTINGS_PREFIX_RIGHT + PreferenceManager.SETTINGS_PANELS_SHOW_TYPE,
						"Full")
				.equalsIgnoreCase("full"))
			rightPanel.getColumn(0).setWidth(rightLabel.getBounds().width - 4);
		return shell;
	...
	protected void storeAllSystemProperties() {
		ConfigHelper conf = ConfigHelper.getInstance();
		conf.setProperty(APPLICATION_PROPERTIES_LEFT_PATH,
				FileHelper.getNearestDirectory(leftLabel.getText().split(ZIP_FILE_AND_PATH_SEPARATOR)[0]));
		conf.setProperty(APPLICATION_PROPERTIES_RIGHT_PATH,
				FileHelper.getNearestDirectory(rightLabel.getText().split(ZIP_FILE_AND_PATH_SEPARATOR)[0]));
	...
	private void loadFavorites(Menu menu) {
		for (FavoriteItemVO item : favorites.getItems()) {
	...
	Menu createHelpMenu() {
		...
		item.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent event) {
				ShowFileDialog dialog = new ShowFileDialog(shell, RESOURCE_DIRECTORY_RELATIVE + RESOURCE_SHORTCUTS_FILE,
		...
		item.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent event) {
				ShowFileDialog dialog = new ShowFileDialog(shell, RESOURCE_DIRECTORY_RELATIVE + RESOURCE_ABOUT_FILE,
						SWT.DIALOG_TRIM | SWT.APPLICATION_MODAL);
				dialog.open();
			}
		});
	...
	private void createPanel(final boolean createLeftPanel) {
		...
		setPanelPreferences((createLeftPanel ? leftPanel : rightPanel),
				createLeftPanel ? PreferenceManager.SETTINGS_PREFIX_LEFT : PreferenceManager.SETTINGS_PREFIX_RIGHT);
		...
		(createLeftPanel ? leftPanelViewer : rightPanelViewer).setCellModifier(new ICellModifier() {
			...
			public void modify(Object element, String property, Object value) {
				...
				if (newName.length() > 0) {
					item.rename(newName);
					refreshPanel((createLeftPanel ? leftLabel : rightLabel).getText(), item.getAbsolutePath(),
							createLeftPanel);
					renameKeysPressed = false;
				}
			}
		...
		int colCount = ConfigHelper.getInstance().getProperty(
				(createLeftPanel ? PreferenceManager.SETTINGS_PREFIX_LEFT : PreferenceManager.SETTINGS_PREFIX_RIGHT)
						+ PreferenceManager.SETTINGS_PANELS_SHOW_TYPE,
				"Full").equalsIgnoreCase("full") ? COLUMN_NAMES.length : 1;


		for (int i = 0; i < colCount; i++) {
			final int ii = i;
			(createLeftPanel ? leftPanelViewer : rightPanelViewer).getTable().getColumn(ii)
		...
		(createLeftPanel ? leftPanelViewer : rightPanelViewer)
					...
					public void selectionChanged(SelectionChangedEvent event) {
						...
						if (item != null)
							if (getPanelItems(lastFocusedLeft).getSelectedItems().length > 0)
								(createLeftPanel ? leftBottomLabel : rightBottomLabel)
										.setText(getPanelItems(lastFocusedLeft).getSelectedItemsStatusText());
							else
	...
	private void setPanelPreferences(Table panel, String preferencePrefix) {
		int colCount = ConfigHelper.getInstance()
				.getProperty(preferencePrefix + PreferenceManager.SETTINGS_PANELS_SHOW_TYPE, "Full")
				.equalsIgnoreCase("full") ? COLUMN_NAMES.length : 1;
		...
		panel.setFont(new Font(shell.getDisplay(), PreferenceConverter.getFontData(
				PreferenceManager.getDefaultPreferenceStore(), preferencePrefix + PreferenceManager.SETTINGS_FONT)));
		panel.setHeaderVisible(ConfigHelper.getInstance()
				.getPropertyAsBoolean(preferencePrefix + PreferenceManager.SETTINGS_SHOW_TABLE_HEADER, true));
		panel.setLinesVisible(ConfigHelper.getInstance()
				.getPropertyAsBoolean(preferencePrefix + PreferenceManager.SETTINGS_SHOW_TABLE_LINES, false));
	}
		...
		newItem.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				ConfigHelper.getInstance()
						.setProperty((lastFocusedLeft ? PreferenceManager.SETTINGS_PREFIX_LEFT
								: PreferenceManager.SETTINGS_PREFIX_RIGHT)
								+ PreferenceManager.SETTINGS_PANELS_SHOW_TYPE, "Brief");
				if (!isBrief()) {
					...
					(lastFocusedLeft ? leftPanelViewer : rightPanelViewer).getTable().getColumn(0)
							.setWidth((lastFocusedLeft ? leftLabel : rightLabel).getBounds().width - 4);
					(lastFocusedLeft ? leftPanelViewer : rightPanelViewer).getTable().getColumn(0).setResizable(false);
					(lastFocusedLeft ? leftPanelViewer : rightPanelViewer)
							.setLabelProvider(new FileLabelProviderBrief());
				}
			}
		});
		...
		newItem.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				ConfigHelper.getInstance()
						.setProperty((lastFocusedLeft ? PreferenceManager.SETTINGS_PREFIX_LEFT
								: PreferenceManager.SETTINGS_PREFIX_RIGHT)
								+ PreferenceManager.SETTINGS_PANELS_SHOW_TYPE, "Full");
				if (isBrief()) {
					(lastFocusedLeft ? leftPanelViewer : rightPanelViewer).getTable().getColumn(0)
							.setWidth(COLUMN_WIDTHS[0]);
					...
					(lastFocusedLeft ? leftPanelViewer : rightPanelViewer)
							.setLabelProvider(new FileLabelProviderFull());
				}
			}
		});
	...
	private void createCommandBar() {
		...
		for (String element : COMMAND_BUTTON_LABELS) {
	...
	protected void handleCommandButton(SelectionEvent e) {
		if (e.getSource() instanceof Button) {
			...
			if (index == COMMAND_BUTTON_EXIT) {
				...
				commandCopy(getSelectedFiles(lastFocusedLeft),
						lastFocusedLeft ? rightLabel.getText() : leftLabel.getText());
			} else if (index == COMMAND_BUTTON_MOVE) {
				commandMove(getSelectedFiles(lastFocusedLeft),
						lastFocusedLeft ? rightLabel.getText() : leftLabel.getText());
	...
	private void commandPackFiles(FileItemVO[] filesToCopy, String targetDir) {
		...
		if (filesToCopy == null || filesToCopy.length < 1)
			return;
		TextDialog dialog = new TextDialog(shell, "Pack files to:",
				targetDir + PATH_SEPARATOR + filesToCopy[0].getNameWithoutExt() + ".zip");
		...
		if (res != null) {
			...
			for (int i = 0; (i < filesToCopy.length) && (!progressDialog.isCancelled()); i++) {
				...
				while (!progressDialog.isCancelled()) {
					if (FileHelper.packFileStructure(progressDialog, file, zout,
							lastFocusedLeft ? leftLabel.getText() : rightLabel.getText(), false)) {
						break;
	...
	private void commandCopy(FileItemVO[] filesToCopy, String targetDir) {
		...
		if (res != null) {
			...
			refreshPanel((lastFocusedLeft ? rightLabel : leftLabel).getText(),
					(lastFocusedLeft ? rightLabel : leftLabel).getText(), !lastFocusedLeft);
			progressDialog.close();
			progressDialog = null;
		}
	}
	...
	private void refreshPanel(FileItemVO item, String oldPath, boolean refreshLeft, int panelIndex) {
		...
		if (item.isInZip()) {
			...
			if (refreshLeft) {
				...
				int selIndex = (panelIndex > -1 ? panelIndex
						: getItemIndexViaAbsolutePath(leftPanel, FileHelper.getZipPathFromLabel(oldPath)));
				leftPanelViewer.getTable().setSelection(selIndex);
				leftBottomLabel.setText(
						((FileItemVO) leftPanel.getItem(leftPanel.getSelectionIndex()).getData()).getStatusLine());
			} else {
				...
				int selIndex = (panelIndex > -1 ? panelIndex
						: getItemIndexViaAbsolutePath(rightPanel, FileHelper.getZipPathFromLabel(oldPath)));
				rightPanelViewer.getTable().setSelection(selIndex);
				rightBottomLabel.setText(
						((FileItemVO) rightPanel.getItem(rightPanel.getSelectionIndex()).getData()).getStatusLine());
			}
			zip.close();
		} else {
			...
			if (refreshLeft) {
				...
				if (rightPanel.getSelectionIndex() > -1)
					rightBottomLabel.setText(((FileItemVO) rightPanel.getItem(rightPanel.getSelectionIndex()).getData())
							.getStatusLine());
			}
		}
	}
	...
	private int getItemIndexViaAbsolutePath(Table panel, String oldPath) {
		...
		for (int i = 0; i < panel.getItemCount(); i++) {
			...
			if (tmp.getAbsolutePath().trim().replace(BACKSLASH_CHAR, ZIP_PATH_SEPARATOR_CHAR)
					.equalsIgnoreCase(oldPath.trim()))
				return i;
		}
		return index;
	}
	...
	private FileItemVO[] getSelectedFiles(boolean onLeftPanel) {
		...
		if (files == null || files.length == 0) {
			...
			files[0] = (FileItemVO) (lastFocusedLeft ? leftPanel.getSelection()[0].getData()
					: rightPanel.getSelection()[0].getData());
		}
		return files;
	}
	...
	/**
	 * Helper class that knows how to display a table widget row for setting Full.
	 */
	...
	/**
	 * Helper class that knows how to display a table widget row for setting Brief.
	 */
	...
	protected class GlobalEventListener implements Listener {
		...
		public void handleEvent(Event event) {
			if (event.keyCode == SWT.F5 && event.stateMask == SWT.ALT && event.widget instanceof Table) {
				commandPackFiles(getSelectedFiles(lastFocusedLeft),
						lastFocusedLeft ? rightLabel.getText() : leftLabel.getText());
				...
				handleFindFiles();
			} else if (event.keyCode == 16777218 && event.stateMask == SWT.CTRL || (event.widget instanceof Table
					&& event.keyCode >= 'A' && event.keyCode <= 'z' && event.stateMask == SWT.NONE)) {
				...
				commandCreateFolder();
			} else if (event.widget instanceof Table && event.stateMask == SWT.NONE
					&& (event.keyCode == SWT.F8 || event.keyCode == SWT.DEL)) {
				commandDelete(getSelectedFiles(lastFocusedLeft));
				...
				if (lastFocusedLeft) {
					...
					if (getPanelItems(lastFocusedLeft).getSelectedItems().length > 0)
						...
						leftBottomLabel
								.setText(((FileItemVO) leftPanel.getItem(leftPanel.getSelectionIndex()).getData())
										.getStatusLine());
				} else {
					...
					if (getPanelItems(lastFocusedLeft).getSelectedItems().length > 0)
						...
						rightBottomLabel
								.setText(((FileItemVO) rightPanel.getItem(rightPanel.getSelectionIndex()).getData())
										.getStatusLine());
				}
			} else if (event.widget instanceof Table && event.character == Constants.ZIP_PATH_SEPARATOR_CHAR
			...
			} else if (event.widget instanceof Table && event.keyCode == SWT.PAGE_DOWN && event.stateMask == SWT.CTRL
	...
	public void browseZipFile() {
		...
		if (fileToBrowse != null && fileToBrowse.length > 0) {
			...
			if (file.isZipFile()) {
				(lastFocusedLeft ? leftLabel : rightLabel)
						.setText(file.getAbsolutePath() + ZIP_FILE_AND_PATH_SEPARATOR + ZIP_PATH_SEPARATOR);
	...
	public Point getPopupLocation(boolean left) {
		Point res = new Point((left ? leftRootButton : rightRootButton).getLocation().x,
				(left ? leftRootButton : rightRootButton).getLocation().y);
	...
	public void handleFindFiles() {
		FindFilesDialog findDialog = new FindFilesDialog(shell, SWT.NONE,
				lastFocusedLeft ? leftLabel.getText() : rightLabel.getText(), this);
	...
	private DragSource createTableDragSource(final Table table) {
		...
		dragSource.addDragListener(new DragSourceListener() {
			...
			public void dragSetData(DragSourceEvent event) {
				if (dndSelection == null || dndSelection.length == 0 || !FileTransfer.getInstance().isSupportedType(event.dataType))
					return;
				...
				item.addSelectionListener(new SelectionAdapter() {
					...
					public void widgetSelected(SelectionEvent event) {
						if (FILE_TYPE_DIR.equals(fileItem.getType())) {
							refreshPanel(fileItem, (lastFocusedLeft ? leftLabel : rightLabel).getText(),
									lastFocusedLeft);
